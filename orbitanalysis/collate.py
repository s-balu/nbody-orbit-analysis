...
def collate_orbit_history(savefile, save_properties=True, verbose=True):
    ...
    with h5py.File(readfile, 'r') as hf:
        ...
        with h5py.File(savefile, 'w') as hf_new:
            ...
            for si, snapshot_number in enumerate(snapshot_numbers):
                ...
                slices, hinds, norb = get_slices(
                    hfsnap, orbiting_offsets_prev, infalling_offsets_prev,
                    halo_ids, si)
                ...
                idx = 0
                orbiting_lengths = []
                for hi, (orbslice_prev, orbslice_new, intslice_prev,
                         intslice_new, countslice) in enumerate(slices):

                    hasprev = False if np.all(np.isnan(orbslice_prev)) else \
                        True

                    slon, slin = slice(*orbslice_new), slice(*intslice_new)

                    if hasprev:
                        slop, slip = slice(*orbslice_prev), slice(
                            *intslice_prev)
                        oids = ids_prev[slop]
                    ...
                    if hasprev:
                        ...
                        ocounts = np.delete(
                            orbit_counts_prev[slice(*countslice)],
                            departed_inds)
                    ...
                    infinds = list(set(range(len(ids_all[slin]))).difference(
                        orbinds))
                    ...
                    if hasprev:
                        ...
                        angles_new = np.arccos(np.einsum(
                            ...
                            coords_all[slin][mask]) / (
                                magnitude(coords_prev[slip][order]) *
                                magnitude(coords_all[slin][mask])))
                        angles[slin][mask] = angles_prev[slip][order] + \
                            angles_new
                ...
                infalling_offsets_prev = offsets_all[:-1] + np.array(
                    orbiting_lengths)
                ids_prev = ids_all
                coords_prev = coords_all
                angles_prev = angles
                orbit_counts_prev = orbit_counts[
                    :int(np.sum(orbiting_lengths))]
